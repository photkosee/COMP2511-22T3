...
public class EntityFactory {
    ...
    public void spawnSpider(Game game) {
        ...
        for (int i = playerPositionX - radius; i < playerPositionX + radius; i++) {
            for (int j = playerPositionY - radius; j < playerPositionY + radius; j++) {
                if (Position.calculatePositionBetween(game.getPlayerPosition(),
                    new Position(i, j)).magnitude() > radius) continue;
    ...
    public Mercenary buildMercenary(Position pos) {
        ...
        double alliedAttack = config.optDouble("ally_attack", Mercenary.DEFAULT_ATTACK);
        double alliedDefence = config.optDouble("ally_defence", Mercenary.DEFAULT_HEALTH);
        return new Mercenary(pos, mercenaryHealth, mercenaryAttack, mercenaryBribeAmount, mercenaryBribeRadius,
                            alliedAttack, alliedDefence);
    }

    public Mercenary buildAssasin(Position pos) {
        ...
        double failRate = config.optDouble("assassin_bribe_fail_rate", 0);
        double alliedAttack = config.optDouble("ally_attack", Mercenary.DEFAULT_ATTACK);
        double alliedDefence = config.optDouble("ally_defence", Mercenary.DEFAULT_HEALTH);
        return new Assassin(pos, assassinHealth, assassinAttack, assassinBribeAmount, assassinBribeRadius, failRate,
                            alliedAttack, alliedDefence);
    }
    ...
    private Entity constructEntity(JSONObject jsonEntity, JSONObject config) {
        ...
        case "assassin":
            return buildAssasin(pos);
        case "light_bulb_off":
            return new LightBulb(pos, LogicFactory.createLogic(jsonEntity.getString("logic")));
        case "wire":
            return new Wire(pos);
        case "switch_door":
            return new SwitchDoor(pos, LogicFactory.createLogic(jsonEntity.getString("logic")));
        default:
            return null;
        }
    }
