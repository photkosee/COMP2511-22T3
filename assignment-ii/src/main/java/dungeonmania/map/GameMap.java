...
public class GameMap implements java.io.Serializable {
    ...
    private void triggerMovingAwayEvent(Entity entity) {
        ...
        getEntities(entity.getPosition()).forEach(e -> {
            if (e != entity && e instanceof MovedAwayable)
            callbacks.add(() -> ((MovedAwayable) e).onMovedAway(this, entity));
        });
    ...
    private void triggerOverlapEvent(Entity entity) {
        ...
        getOverlapableEntities(entity.getPosition()).forEach(e -> {
            if (e != entity)
    ...
    public List<ConductorEntity> getConductors(Position p) {
        GraphNode node = nodes.get(p);
        return (node != null) ? node.getConductors() : new ArrayList<>();
    }

    public List<Overlapable> getOverlapableEntities(Position p) {
        GraphNode node = nodes.get(p);
        return (node != null) ? node.getOverlabableEntities() : new ArrayList<>();
    }
    ...
    public void gameUnsubscribe(String id) {
        game.unsubscribe(id);
    }
